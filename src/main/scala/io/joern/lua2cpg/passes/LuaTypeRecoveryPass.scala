...
class LuaTypeRecoveryPass(cpg: Cpg, config: XTypeRecoveryConfig = XTypeRecoveryConfig(iterations = 3))
  ...
  override protected def generateRecoveryPass(state: XTypeRecoveryState): XTypeRecovery[NamespaceBlock] =
    new LuaTypeRecovery(cpg, state)
}

private class LuaTypeRecovery(cpg: Cpg, state: XTypeRecoveryState) extends XTypeRecovery[NamespaceBlock](cpg, state) {
  ...
  override def generateRecoveryForCompilationUnitTask(
    ...
    new RecoverForLuaFile(cpg, unit, builder, state.copy(config = newConfig))
  }
}
